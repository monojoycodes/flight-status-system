...
const scheduleSchema = new mongoose.Schema(
  {
    ...
    status: {
      type: String,
      enum: ["scheduled", "boarding", "check-in", "delayed", "cancelled", "security"],
      default: "scheduled",
      required: true
    }
  },
...
scheduleSchema.index({ flightNumber: 1, departureTime: 1 }, { unique: true });

scheduleSchema.pre("save", function () {
  if (this.actualTime && this.departureTime) {
    const delay = (this.actualTime - this.departureTime) / (1000 * 60);
